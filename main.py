...
class Game:
    def __init__(self):
        ...
        self.GRIDWIDTH = start_width / TILESIZE
        self.GRIDHEIGHT = start_height / TILESIZE
        ...
        for x in range(10):
            surf = pygame.Surface((self.bigest_num_length,self.BIG_TEXT*1.2), pygame.SRCALPHA)
    ...
    def draw_start_game_screen(self,loading=False):
        ...
        if self.game_status == PLAYER_DIED:
            self.draw_text(self.screen, "GAME OVER", self.GIANT_TEXT,self.WIDTH / 2, int(self.HEIGHT * 0.13), rect_place="mitte", color=AUSWAHL_TEXT_RED)
        elif self.game_status == WON_GAME:
            self.draw_text(self.screen, "YOU WON",   self.GIANT_TEXT,self.WIDTH / 2, int(self.HEIGHT * 0.13), rect_place="mitte", color=AUSWAHL_TEXT_GREEN)
        else:
            self.draw_text(self.screen, "Zombie!",   self.GIANT_TEXT,self.WIDTH / 2, int(self.HEIGHT * 0.13), rect_place="mitte", color=AUSWAHL_TEXT_COLOR)

        # Schwierigkeit
        circle_size = self.calculate_fit_size(0.02604167,0.0390625)
        self.draw_text(self.screen,"Schwierigkeit",int(self.BIG_TEXT*1.2),self.WIDTH/2,int(self.HEIGHT*0.25),color=AUSWAHL_TEXT_COLOR)
        ...
        # Spielmodus
        self.draw_text(self.screen, "SPIELMODUS", int(self.BIG_TEXT*1.2), self.WIDTH / 2, int(self.HEIGHT * 0.47), color=AUSWAHL_TEXT_COLOR)
        if self.spielmodus != MAP_MODUS or loading:
            return_dict[MAP_MODUS] = self.draw_text(self.screen, "Zombie Map", self.NORMAL_TEXT, self.WIDTH*2/3, int(self.HEIGHT * 0.57), color=AUSWAHL_TEXT_COLOR, rect_place="mitte")
            if loading and self.spielmodus == MAP_MODUS:
                spielmodus_rect = return_dict[MAP_MODUS]
        elif self.spielmodus == MAP_MODUS:
            spielmodus_rect = self.draw_text(self.screen, "Zombie Map", self.NORMAL_TEXT, self.WIDTH*2/3, int(self.HEIGHT * 0.57), color=AUSWAHL_TEXT_GREEN, rect_place="mitte")
            return_dict[MAP_MODUS] = spielmodus_rect
        if self.spielmodus != ARENA_MODUS or loading:
            return_dict[ARENA_MODUS] = self.draw_text(self.screen, "Arena Modus", self.NORMAL_TEXT, self.WIDTH*1/3, int(self.HEIGHT * 0.57), color=AUSWAHL_TEXT_COLOR, rect_place="mitte")
            if loading and self.spielmodus == ARENA_MODUS:
                spielmodus_rect = return_dict[ARENA_MODUS]
        elif self.spielmodus == ARENA_MODUS:
            spielmodus_rect = self.draw_text(self.screen, "Arena Modus", self.NORMAL_TEXT, self.WIDTH*1/3, int(self.HEIGHT * 0.57), color=AUSWAHL_TEXT_GREEN, rect_place="mitte")
        ...
        if self.spielmodus == MAP_MODUS:
            ...
            if self.genauerer_spielmodus == AFTER_TIME and not loading:
                return_dict[AFTER_TIME+"0"] = self.draw_text(self.screen, "Gewonnen nach",      self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
                return_dict[AFTER_TIME+"1"] = self.draw_text(self.screen, "Zeit",               self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
            else:
                return_dict[AFTER_TIME+"0"] = self.draw_text(self.screen, "Gewonnen nach",      self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
                return_dict[AFTER_TIME+"1"] = self.draw_text(self.screen, "Zeit",               self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
            if self.genauerer_spielmodus == AFTER_KILLED and not loading:
                return_dict[AFTER_KILLED+"0"] = self.draw_text(self.screen, "Gewonnen nach",      self.NORMAL_TEXT, self.WIDTH * 3 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
                return_dict[AFTER_KILLED+"1"] = self.draw_text(self.screen, "töten aller Zombies",self.NORMAL_TEXT, self.WIDTH * 3 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
            else:
                return_dict[AFTER_KILLED+"0"] = self.draw_text(self.screen, "Gewonnen nach",      self.NORMAL_TEXT, self.WIDTH * 3 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
                return_dict[AFTER_KILLED+"1"] = self.draw_text(self.screen, "töten aller Zombies",self.NORMAL_TEXT, self.WIDTH * 3 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
        elif self.spielmodus == ARENA_MODUS:
            ...
            if self.genauerer_spielmodus == AFTER_TIME:
                return_dict[AFTER_TIME+"0"] = self.draw_text(self.screen, "Zombiewelle nach",   self.NORMAL_TEXT, self.WIDTH * 1 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
                return_dict[AFTER_TIME+"1"] = self.draw_text(self.screen, "Zeit",               self.NORMAL_TEXT, self.WIDTH * 1 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
            else:
                return_dict[AFTER_TIME+"0"] = self.draw_text(self.screen, "Zombiewelle nach",   self.NORMAL_TEXT, self.WIDTH * 1 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
                return_dict[AFTER_TIME+"1"] = self.draw_text(self.screen, "Zeit",               self.NORMAL_TEXT, self.WIDTH * 1 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
            if self.genauerer_spielmodus == AFTER_KILLED:
                return_dict[AFTER_KILLED+"0"] = self.draw_text(self.screen, "Zombiewelle nach",   self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
                return_dict[AFTER_KILLED+"1"] = self.draw_text(self.screen, "töten aller Zombies",self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_GREEN, rect_place="oben_mitte")
            else:
                return_dict[AFTER_KILLED+"0"] = self.draw_text(self.screen, "Zombiewelle nach",   self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62), color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")
                return_dict[AFTER_KILLED+"1"] = self.draw_text(self.screen, "töten aller Zombies",self.NORMAL_TEXT, self.WIDTH * 2 / 4, int(self.HEIGHT * 0.62) + self.NORMAL_TEXT + 5, color=AUSWAHL_TEXT_COLOR, rect_place="oben_mitte")

        # Karte
        self.draw_text(self.screen, "KARTE", int(self.BIG_TEXT*1.2), self.WIDTH / 2, int(self.HEIGHT * 0.74), color=AUSWAHL_TEXT_COLOR)
        for map_count,karten_name in enumerate(MAP_NAMES):
            if self.map_name == karten_name and not loading:
                return_dict["Map"+str(map_count)] = self.draw_text(self.screen, karten_name, self.NORMAL_TEXT, self.WIDTH * (map_count+1) / (len(MAP_NAMES)+1), int(self.HEIGHT * 0.84), color=AUSWAHL_TEXT_GREEN, rect_place="mitte")
            else:
                return_dict["Map"+str(map_count)] = self.draw_text(self.screen, karten_name, self.NORMAL_TEXT, self.WIDTH * (map_count+1) / (len(MAP_NAMES)+1), int(self.HEIGHT * 0.84), color=AUSWAHL_TEXT_COLOR, rect_place="mitte")

        if loading:
            self.draw_text(self.screen, "Lädt ...", self.HUGE_TEXT, self.WIDTH / 2, int(self.HEIGHT * 0.94), rect_place="mitte",color = AUSWAHL_TEXT_RED)
        else:
            return_dict["Spielen"] = self.draw_text(self.screen, "Spielen", self.HUGE_TEXT, self.WIDTH / 2, int(self.HEIGHT * 0.94), rect_place="mitte",color = AUSWAHL_TEXT_COLOR)
        pygame.display.flip()
    ...
    def make_lehrer_selection_pictures(self):
        if self.multiplayer:
            lehrer_asuwahl_breite = self.WIDTH / self.num_players_in_multiplayer
        else:
        ...
        for lehrer in LEHRER_NAMEN:
            ...
            #  #########################   ####################   ###############
            #  #Herr Lifka () █ █ |   |#   #Herr Lifka () █ █ #   #Herr Lifka ()#
            ...
            infotexte = [str(LEHRER[lehrer]["player_health"]),"Geschw:" + str(LEHRER[lehrer]["player_speed"]),"Powerup zeit:" + str(round(LEHRER[lehrer]["power_up_time"] / 1000,1)),"Nachladezeit:" + str(LEHRER[lehrer]["weapon_rate"]),"Ungenauigkeit:" + str(LEHRER[lehrer]["weapon_spread"]),"Schussweite:" + str(round((LEHRER[lehrer]["weapon_bullet_speed"]* LEHRER[lehrer]["weapon_lifetime"]) / 1000)),"Schaden:" + str(LEHRER[lehrer]["weapon_damage"])]
    ...
    def draw_lehrer_selection(self,surf,selected,player_num,such_text=""):
        ...
        if self.multiplayer:
            linker_rand = self.WIDTH / len(self.players) * player_num
            lehrer_asuwahl_breite = self.WIDTH / len(self.players)
        else:
            linker_rand = 0
            lehrer_asuwahl_breite = self.WIDTH

        subsurface = self.background.subsurface((linker_rand, 0, self.WIDTH / len(self.players), self.HEIGHT))
    ...
    def draw_display(self):
        ...
        for count,camera in enumerate(self.camera):
            ...
            screen_part = pygame.Rect((self.WIDTH / len(self.players) * count, 0), (self.WIDTH / len(self.players), self.HEIGHT))
        ...
        if self.multiplayer:
            for player_num in range(len(self.players)):
                if player_num != 0:
                    pygame.draw.line(self.screen, BLIT_SCREEN_LINE_COLOR, (self.WIDTH / len(self.players) * player_num, 0), (self.WIDTH / len(self.players) * player_num, self.HEIGHT), 8)
        time3 = self.make_time_measure()
        # Live Bar
        for count,player in enumerate(self.players):
    ...
    def update_forground_text_img(self):
        ...
        # Texte
        if self.multiplayer:
            rect = self.draw_text(surface, 'Zombies: ', self.BIG_TEXT, self.WIDTH / 2 - (3*self.bigest_num_length)/2, 10, rect_place="oben_mitte")
            self.num_zombies_text_pos = (self.WIDTH / 2 - (3*self.bigest_num_length)/2 + rect.width/2, 10)
            for count, player in enumerate(self.players):
                if count >= self.num_players_in_multiplayer/2:
                    rect = self.draw_text(surface, LEHRER[player.lehrer_name]["personen_item_text"] + ": ", self.BIG_TEXT, self.WIDTH - ((self.WIDTH/self.num_players_in_multiplayer)*(self.num_players_in_multiplayer-count-1)) - 10 - 2*self.bigest_num_length, self.HEIGHT - 30 - self.HEIGHT * 0.02,rect_place="unten_rechts")
                    self.personen_item_text_pos.append((self.WIDTH - ((self.WIDTH/self.num_players_in_multiplayer)*(self.num_players_in_multiplayer-count-1)) - 10 - 2*self.bigest_num_length,self.HEIGHT - 30 - self.HEIGHT * 0.02))
                else:
                    rect = self.draw_text(surface, LEHRER[player.lehrer_name]["personen_item_text"] + ": ", self.BIG_TEXT, 10 + ((self.WIDTH/self.num_players_in_multiplayer)*(count)), self.HEIGHT - 30 - self.HEIGHT * 0.02,rect_place="unten_links")
                    self.personen_item_text_pos.append((10 + ((self.WIDTH/self.num_players_in_multiplayer)*(count)) + rect.width, self.HEIGHT - 30 - self.HEIGHT * 0.02))
        else:
        ...
        if self.spielmodus == ARENA_MODUS:
            if self.multiplayer:
                self.draw_text(surface, 'Welle: ' + str(self.num_zombie_wave), self.BIG_TEXT, self.WIDTH / 2, self.HEIGHT - 20 - self.HEIGHT * 0.02 - 3 -self. BIG_TEXT - 10, rect_place="oben_mitte")
            else:
                self.draw_text(surface, 'Welle: ' + str(self.num_zombie_wave), self.BIG_TEXT, 10, self.HEIGHT - 20 - self.HEIGHT * 0.02 - 3 - self.BIG_TEXT - 10, rect_place="oben_links")
    ...
    def update_live_bar_image(self, player, player_num):
        surface = pygame.Surface((self.live_bar_img_width+self.WIDTH/3,self.live_bar_img_height+self.HEIGHT/4), pygame.SRCALPHA)
        ...
        for count, text in enumerate(beschreibungs_texte):
            if self.multiplayer and player_num >= self.num_players_in_multiplayer/2:
                self.draw_text(surface, beschreibungs_texte[count], self.SMALL_TEXT, bild_breite - 0.569444 * self.live_bar_img_width,0.2 * self.live_bar_img_height + self.NORMAL_TEXT + count * (self.SMALL_TEXT + 5),
                               rect_place="oben_rechts", font_name=ARIAL_FONT, color=BLACK)
            else:
                self.draw_text(surface, beschreibungs_texte[count], self.SMALL_TEXT, 0.569444 * self.live_bar_img_width,0.2 * self.live_bar_img_height + self.NORMAL_TEXT + count * (self.SMALL_TEXT + 5), rect_place="oben_links",
                               font_name=ARIAL_FONT, color=BLACK)
    ...
    def draw_live_bar(self,player,player_num):
        ...
        if self.multiplayer and player_num >= self.num_players_in_multiplayer/2:
            fill_rect = pygame.Rect(int(self.WIDTH-((self.WIDTH/self.num_players_in_multiplayer)*(self.num_players_in_multiplayer-player_num-1))-int(BAR_LENGTH)-int(0.031 * self.live_bar_img_width)), int(0.0426829 * self.live_bar_img_height), int(pct * BAR_LENGTH), int(BAR_HEIGHT))
        else:
